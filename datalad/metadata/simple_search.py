...
@build_doc
class SimpleSearch(Interface):
    """Search within available in datasets' meta data
    """
    # make the custom renderer the default, this whole thing is an alien
    # priority here
    ...
    @staticmethod
    @datasetmethod(name='simple_search')
    @eval_results
    def __call__(match,
                 dataset=None,
                 search=None,
        ...
        if meta is None:
            ...
            meta = ds.metadata(recursive=True, reporton='datasets', return_type='list')
        ...
        for mds in meta:
            ...
            # manual loop for now
            for k, v in iteritems(mds.get('metadata', {})):
                if search:
                    k_lower = k.lower()
                ...
                if isinstance(location, (list, tuple)):
                    ...
                    for l in location:
                        yield get_status_dict(
                            path=opj(ds_path_prefix, l),
                            status='ok',
                            action="simple_search",
                            query_matched=report_dict)
                else:
                        yield get_status_dict(
                            path=opj(ds_path_prefix, location),
                            status='ok',
                            action="simple_search",
                            query_matched=report_dict)
    ...
    def custom_result_renderer(res, **kwargs):
        ...
        if kwargs['report'] in ('*', ['*']) \
                or kwargs['report_matched'] \
                or (kwargs['report'] is not None
                    and len(kwargs['report']) > 1):
