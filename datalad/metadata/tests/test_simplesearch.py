...
from datalad.api import simple_search
from datalad.metadata import simple_search as search_mod
...
def test_search_outside1_install_central_ds(tdir, central_dspath):
    with chpwd(tdir):
        ...
        with \
            ...
            patch('datalad.distribution.dataset.Dataset.simple_search',
...
def test_our_metadataset_search(tdir):
    ...
    assert list(ds.simple_search('.', report='*', regex=True))
    assert list(ds.simple_search('.', report='*'))
    assert list(ds.simple_search('.', report_matched=True))
    ...
    out, err = Runner(cwd=tdir)('datalad simple-search Buzsáki')
...
def test_search_non_dataset(tdir):
    ...
    with assert_raises(NoDatasetArgumentFound) as cme:
        list(simple_search('smth', dataset=tdir))
