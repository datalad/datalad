...
def test_digester(path):
    digester = Digester()
    assert_equal(digester(opj(path, 'sample.txt')), SAMPLE_DIGESTS['123'])
    assert_equal(digester(opj(path, '0')), SAMPLE_DIGESTS[chr(0)])
    assert_equal(digester(opj(path, 'long.txt')), SAMPLE_DIGESTS['__long__'])
