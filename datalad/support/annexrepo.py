...
class AnnexRepo(GitRepo):
    ...
    def annex_proxy(self, git_cmd):
        ...
        try:
            out, err = self.cmd_call_wrapper(cmd_str, shell=True, return_output=True, cwd=self.path)
    ...
    def get_file_key(self, files):
        ...
        try:
            out, err = self.cmd_call_wrapper.run(cmd_list, return_output=True, cwd=self.path)
        except CommandError, e:
            if e.code != 1:
            ...
            if not exists(opj(self.path, path)):
                ...
                raise FileInGitError(cmd=cmd_str,
                                     msg="File not in annex, but git: %s" % path,
                                     filename=path)
            else:
                raise FileNotInAnnexError(cmd=cmd_str,
                                          msg="File not in annex: %s" % path,
                                          filename=path)
        return out.split(linesep)[0]
