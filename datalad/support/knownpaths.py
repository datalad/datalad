...
#
# ## ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ##
"""Python wrapper around the SHGetKnownFolderPath Windows Shell function as an
alternative to appdirs (that uses outdated CSIDL IDs) to determine Special
Folders under Windows. Was shared under an MIT license in
...
def get_path(folderid, user_handle=UserHandle.current):
    """Get special directory paths under windows

    Usage:
    # get local app data dir for the current user
    from datalad.support.knownpaths import get_path, FOLDERID
    path = get_path(getattr(FOLDERID, "LocalAppData"))
